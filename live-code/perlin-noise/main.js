...
const drawQuad = regl({
    vert: `
        ...
        void main() {
			vPosition = positions;
            gl_Position = projection * view * model * vec4(positions, 0., 1.);
        }
    `,
    frag: `
		...
		vec3 clouds(float x, float y) {
			float L = turbulence(vec3(x, y, time * .15));
			return vec3(noise(vec3(.5, .5, L) * .7));
		}

        void main() {
			vec3 color = clouds(vPosition.x, vPosition.y) + vec3(0.5, 0.8, 0.95);
            gl_FragColor = vec4(color, 1.0);
        }
    `,
    uniforms: {
        // Object scale. orientation and translation
        model: () => {
            const scale = mat4.fromScaling([], [0.1, 0.1, 1])
            return scale
        },
...
// Handles requestAnimationFrame() looping
regl.frame(() => {
